...
import { ChevronLeft, ChevronRight, Filter, ChevronDown, Check, Zap } from 'lucide-react';
import { SKILLS_DATA } from './SkillsData';
import { SkillsProps } from './SkillsTypes';
import { Button } from '@/components/ui/button';

const ITEMS_PER_PAGE = 12;
const CATEGORIES = ['All', 'Development', 'Design', 'IT Support', 'Data Analysis', 'Tools'];
...
const Skills: React.FC<SkillsProps> = () => {
  ...
  const filteredItems = useMemo(() => {
    return activeFilter === 'All' 
      ? SKILLS_DATA 
      : SKILLS_DATA.filter(item => item.category === activeFilter);
  }, [activeFilter]);
    ...
    <section id="skills" className="py-20 bg-dark-900 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="font-heading text-3xl md:text-4xl font-bold mb-4">Skills & Tools</h2>
          <p className="text-gray-400 max-w-2xl mx-auto">
            A comprehensive set of technologies and tools I use to build and manage digital solutions.
          </p>
        </motion.div>
        ...
        <div className="lg:hidden relative mb-8 z-30 max-w-xs mx-auto">
          <button
            ...
            className="w-full bg-dark-800 border border-white/10 rounded-xl px-4 py-3 text-left flex items-center justify-between text-white shadow-lg focus:outline-none focus:border-primary-500"
          >
            ...
            {isDropdownOpen && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute top-full left-0 right-0 mt-2 bg-dark-800 border border-white/10 rounded-xl shadow-2xl overflow-hidden z-40 max-h-60 overflow-y-auto custom-scrollbar"
                >
                  {CATEGORIES.map((category) => (
                    <button
                      key={category}
                    ...
                    >
                      {category}
                      {activeFilter === category && (
                        <Check className="w-4 h-4 text-primary-400 group-hover:text-white" />
                      )}
                    </button>
                  ))}
                </motion.div>
            ...
            >
              {currentItems.map((item, index) => (
                <motion.div 
                  key={`${item.name}-${index}`}
                  whileHover={{ y: -5 }}
                  className="bg-dark-800/50 border border-white/5 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:border-primary-500/30 hover:bg-white/5 transition-all duration-300 group h-full"
                >
                  <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center mb-4 group-hover:bg-primary-500/20 group-hover:scale-110 transition-all duration-300">
                    <item.icon className="w-6 h-6 text-gray-400 group-hover:text-primary-400 transition-colors" />
                  </div>
                  
                  <h3 className="text-base font-bold text-white mb-1 group-hover:text-primary-300 transition-colors">
                    {item.name}
                  </h3>
                  <span className="text-xs text-gray-500 bg-white/5 px-2 py-0.5 rounded border border-white/5 mt-2">
                    {item.category}
                  </span>
                </motion.div>
              ))}
